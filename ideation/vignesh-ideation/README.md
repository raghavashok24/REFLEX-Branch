...
**In plain terms.** REFLEX showed that a single automated trader can spiral out of control when its own trades reshape the market it then learns from, and it captured that with one stability formula for the bond market. This paper takes that one formula and turns it into a general-purpose toolkit for any self-affecting learning system, plus a practical test that says whether a system is safely stable, unstable, or too-close-to-call given only limited real data. Its main improvement over REFLEX is scope and usefulness: instead of describing one market, it lets you check and actively control stability by tuning how often the system retrains.
...
**In plain terms.** REFLEX also noticed that when several traders share one market, they amplify each other's behavior. This paper zooms in on that group effect and asks when a crowd of individually safe, well-behaved traders can still tip the whole market into instability just by interacting. Its improvement over REFLEX is to turn that side observation into an economics framework: it treats the problem as a "learning externality" and shows that the number of competing agents itself becomes a systemic risk factor, even when no single agent is doing anything wrong.
