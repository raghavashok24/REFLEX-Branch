...
A firm's marginal effect on the market is $\partial\mN/\partial m_i=\Neff v_i^2$, where $v$ is the leading eigenvector of the coupling matrix. These shares add up to $\Neff$, not to one. On exchangeable $R$ each is $\Neff/N$, so a naive commons guess of $1/N$ is off by a factor of $\Neff$, which is $16.2$ at $N=20$, $\kappa=0.8$ and $s=1$.

\begin{theorem}[The wedge]
\label{thm:wedge}
At a symmetric profile, the private and social first-order conditions differ only in the price of crowding. The marginal cost a firm ignores is
\begin{equation}
...
We reran the shared-pool market from our earlier work \cite{reflex2026} and measured common-mode moduli of $0.7856$, $1.3692$ and $2.4799$ at $N=1,2,3$. That is amplification of $1.7428\times$ and $3.1567\times$, matching the published $1.74\times$ and $3.16\times$ exactly. This check covers the monoculture case $R=\one\one^{\top}$, so it supports the base law the paper builds on, not any of the four new results. It is also our own earlier work, so it is not independent validation.
