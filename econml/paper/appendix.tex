...
$\lmax(R)$ & effective number of independent learners \emph{(new)} & $[1,N]$ \\
...
$\gamma_{PO}$ & curvature governing the corrected dynamics & $>\gamma$ \\
...
$\rho^\ast$ & herd-immunity threshold \emph{(new)} & $[0,1)$ \\
...
This appendix proves Lemma~\ref{lem:reduction} and Theorem~\ref{thm:alignment}, establishes the properties of $R$ they use, gives the three anchors, and proves that mean-based diversity indices err with a sign.
...
At $\kappa = s = 1$ the quadratic degenerates, since $1-k_s = 0$ kills $a$, $b$ and $Q$, leaving $\rho = P = m_1(N_b + \theta N_{\text{corr}})$. Writing the corrected fraction as $\rho_c = 1 - N_b/N$ and solving $\rho < 1$ for it gives
\[
  \rho^\ast(\theta) \;=\; \frac{1 - 1/\mN}{1-\theta} \;=\; \frac{1-1/\mN}{e}, \qquad e = 1-\theta = 1 - \gamma/\gamma_{PO},
\]
...
Certificate C14 finds zero mismatches on 4000 draws for this form. The clamped restatement is \emph{not} equivalent: writing $\rho_c = 1-N_b/N$ and $\rho^\ast = \max(0,\,1-N_c(s)/N)$, the criterion $\rho_c > \rho^\ast$ fails on 134 of those same 4000 draws, every failure at the same corner, namely $N_b = N$ with $N_c(s) \geq N$, the all-blind market that is stable because it needs no correction at all and that a clamp at zero combined with a strict inequality excludes. The paper therefore states $N_b < N_c(s)$ as the theorem, which is primitive and exact, and presents the clamped $\rho^\ast$ as the policy object, read as the fraction that must be corrected, with $\rho^\ast = 0$ meaning no requirement rather than a strict inequality to clear.

Since $\rho^\ast N$ is generally not an integer and the panels run at $N$ in the tens, where one firm is several percentage points, the realized requirement is
\[
  \text{minimum corrected firms} \;=\; N - \lceil N_c(s)\rceil + 1,
\]
clamped to $[0,N]$, because the largest stable blind count is $\lceil N_c\rceil - 1$. The natural guess $\lceil \rho^\ast N\rceil$ agrees on every random draw, since random draws never land on an exact integer $N_c$, but it is off by one exactly when $N_c$ is an integer: at $N=20$, $m_1=0.15$, $\kappa=0.8$ and the $s$ giving $N_c=8$, the truth is 13 corrected firms and the guess says 12. Certificate H4 checks the correct form exactly on 3000 draws. Worked at $N=20$, $m_1=0.15$ and $\kappa=0.8$:

\begin{center}
\small
\begin{tabular}{llll}
\toprule
$s$ & $N_c(s)$ & $\rho^\ast$ & minimum corrected firms \\
\midrule
$1.0$ & $8.08$ & $0.596$ & $12$ \\
$0.5$ & $15.17$ & $0.242$ & $5$ \\
$0.2$ & $36.42$ & $0$ & $0$ \\
...
At $\kappa = s = 1$ we have $N_c = 1/m_1$ and $\Neff = N$, so $\rho^\ast = 1 - 1/(m_1N) = 1 - 1/\mN$, the epidemiological threshold with the systemic modulus as the reproduction number: a market of ten firms at $\mN = 2.5$ needs $60\%$ un-blinded. And $\rho^\ast$ is increasing in $s$, strictly so wherever it is positive, which is what makes model diversity and corrected learning substitutes and what the $(\rho,s)$ frontier plots. Certified in C15 and C16.
...
\item[(W3) Exchangeable alignment.] $R$ is invariant under a common permutation of firms. The supply-chain alignment is exchangeable, as are the monoculture and orthogonal corners. This buys the uniform leading eigenvector at the symmetric point.
...
\begin{aproposition}
\label{aprop:cstat}
$t^\ast$ is strictly increasing in $N$, in $\kappa$ and in $\mN$, and diverges as $\mN \to 1$.
\end{aproposition}
...
\midrule
525 & total & all passing at submission \\
\bottomrule
\end{tabular}
\end{center}
...
\paragraph{The panels.} Panel 1 sweeps $N$ in the homogeneous shared-pool market and reproduces the published amplification, which is external validation against a prior run rather than a self-consistency check, and it is the paper's one measured result. Panel 2 measures $\mN$ over a grid of firms by shared-model fraction against the predicted boundary, with firms placed at the target alignment exactly rather than in distribution so that an $s$ sweep moves along the exact curve; its clustered companion, three aligned firms among ten, is the topology worked in Appendix~\ref{app:alignment}. Panel 3 sweeps the $(\Neff, K)$ grid against $K_{\max}$ at $c = 0.8$, including the critical-crowding column where the window closes. Panel 4 runs mixed markets at corrected fractions $0, 1/N, \ldots, 1$ in an unstable market. Panel 5 traces the $(\rho, s)$ iso-stability curve, and it is the paper's most policy-legible object. Panel 6 solves both first-order conditions by bisection with $\mN$ read from the reference environment's realized dynamics rather than from the closed form, so the over-adaptation verdict is a property of the simulated market.
