...
% Add ",final" to the option list at camera-ready (removes line numbers and
% prints the workshop name in the footer).
...
\item \textbf{Minimax floors (\S\ref{sec:price}).} No non-anticipating policy beats the floor: exactly under deviation budgets; within a factor $\le 13.5$ under value budgets, with the sharp constant numerically supported but open (Theorem~\ref{thm:devbudget}). Adversarial numerics show the floor is \emph{structure-proof} (no family-knowing design beats it; best found $1.005\times$) and map its exact boundary: curvature uncertainty protects it.
...
\label{thm:devbudget}
(i) Over all non-anticipating (possibly randomized, adaptive) policies with pathwise deviation budget $\sum_{t\le T}\dev_t^2 \le D$, all estimators included, and any prior of width $w$: $\inf\sup\Var(\hat\eps) \ge \sigma^2/(D + \sigma^2(\pi/w)^2)$, so the product floor is attained up to the prior term, with equality by symmetric probing. (ii)~Under a budget on realized performative cost with certainty-equivalent anchoring, the floor holds within a constant factor over adaptive policies: $\sup \Var(\hat\eps)\times C_T \ge \gpo\sigma^2/27\,(1-O(T^{-1/2}))$ by a Le~Cam two-point reduction, the exploited rebate vanishing at rate $T^{-1/2}$ (Lemma L2, Appendix~\ref{app:theorems}). The sharp constant $\tfrac{1}{2}$ is numerically supported and open (OPEN-1).
\end{theorem}

Closing the sharp constant is open (OPEN-1); we attacked its premise adversarially. Minimizing the Cram\'er--Rao product over all designs \emph{within the known three-parameter family} finds nothing below the floor (best $1.005\times$, both cost models), the optimizer collapsing back to the symmetric two-point configuration the proof strategy's least-favorable reduction needs; tight-side probing (sensitivity $e^{-ch}$) is monotonically \emph{worse} (ratio $5.7 \to 41$): remote information must be pulled back through $\hat c$, and extrapolation variance dominates the sensitivity gain. The boundary is exact: with $c$ known a priori the floor breaks outright, and the best design's Cram\'er--Rao product falls monotonically as the probe widens (ratio $0.30$ at $t{=}1.0$, $0.04$ at $t{=}0.05$). \emph{The floor is an ignorance-of-curvature phenomenon}: the correct scope for the general theorem.
...
\end{minipage}
\caption{\textbf{Left:} measured $\Var(\hat\eps)\times C_T$ vs amplitude, two moduli: flat on the invariant (dashed), small drift as A1 predicts. \textbf{Middle:} the anchoring crossover (T7): structure wins until misspecification crosses the threshold. \textbf{Right:} certification before exploitation: the agent holds near $\hsp$ for ${\sim}1{,}300$ deployments while the confidence sequence tightens (shaded: frozen), then steps to $\hpo$; certification time is the exchange rate at work.\label{fig:gate}}
\label{fig:exchange}
\end{figure}
...
All results are deterministic from seeds, numpy-only, CPU-only; reported final errors are $300$-step tail averages over the deployed path, stable to three decimals under horizon perturbation. The repository contains: the derivation documents and theorem register (25 results; 34 numerical derivation checks), the pipeline (environments, estimators, designs, agents, baselines; 36 measured-vs-predicted rows; 9 unit tests), the real-data leg with its 10/10 port validation, and continuous integration running the verification suites on every push. The register of ten measurement-forced pivots, each recorded in the code where it happened, with the failed prediction and the fix, is in the repository README.
