...
| P9.3 (multi-bond `Gamma_PO`) | Bergault-Gueant factor reduction; REFLEX 1.5 | Pure instantiation: extends the base project's curvature machinery to the performative objective so every d-dimensional constant in T5/C5.1 is computable at `O(d k^2)`. | - |
