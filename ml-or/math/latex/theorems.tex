...
% ---- macros (keep identical in the NeurIPS build) ----
...
any non-anticipating policy under the certainty-equivalent anchor, the
expected first-order gain obeys
\[
\E[\mathrm{gain}_T] \;\le\; \frac{g_1\,\delta^2}{\sigma}\,
S_T\,\sqrt{T},
\qquad g_1 = \beta\,|\hstar - \psi|,
\]
via the posterior-imbalance bound
$\E\,\big|\,\E[s\mid\mathcal{F}_t]\,\big| \le \delta\sqrt{S_t}/\sigma$
(Pinsker): exploitation is bounded by the information already purchased.
\end{lemma}

\begin{theorem}[Minimax lower bound, value budget; T4]
At the minimax scale $\delta = c\,\sigma/\sqrt{S_T}$, under
stationary-scale exploration,
\[
\inf_{\text{policy, estimator}}\;\sup\;\;
\Var(\hat\eps)\times C_T \;\ge\; \tfrac{1}{2}\,\gpo\,\sigma^2\,
\big(1 - O(T^{-1/2})\big),
\]
with the explicit constant $2 g_1 c^2 \sigma/(\gpo v)$ in the correction
term.
\end{theorem}
