...
def c3_latex():
    print("== C3: latex/theorems.tex structure ==")
    tex = read(os.path.join("latex", "theorems.tex"))
    ...
    for tok, env in re.findall(r"\\(begin|end)\{(\w+\*?)\}", tex):
        if tok == "begin":
            ...
            if stack:
                stack.pop()
    check("environments balanced and properly nested",
    ...
    while i < len(body):
        ...
        i += 1
    check("braces balanced", ok_braces and depth == 0, "final depth %d" % depth)
    n_disp = len(re.findall(r"\\\[", body)), len(re.findall(r"\\\]", body))
    check("display math balanced", n_disp[0] == n_disp[1],
          r"%d \[ vs %d \]" % n_disp)
    n_thm = len(re.findall(r"\\begin\{(theorem|lemma|corollary|proposition)\}", tex))
    check("statement count matches register core (>= 16)", n_thm >= 16,
          "%d theorem-like environments" % n_thm)
