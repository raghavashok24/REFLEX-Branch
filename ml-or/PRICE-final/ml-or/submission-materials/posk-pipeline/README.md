# posk - the price-of-self-knowledge pipeline (paper: PRICE)

The ML architecture and full pipeline for the ML x OR @ NeurIPS 2026 paper
*"PRICE: Minimax Estimation-Regret Tradeoffs and Certified Exploration
in Performative Prediction"* - the companion
...
run repeatedly during development; **eleven pivots were forced by
...
8. **Real data, the 75.8x config trap.** The first port of the REFLEX
   ...
   0.8%). The validation harness is what caught it - the point of
   having one.
...
10. **OPEN-1, the floor looked structure-proof (premise check).** The
   first adversarial search (ratio 1.005, tight probes monotonically
   worse) suggested no family-knowing design beats the exchange-rate
   floor. See pivot 11 for what a proof, and a harder search, then
   showed.
11. **T9, the reach is exact - and the old verdict was a search
   artifact.** Proving structure-proofness (T9: pointwise domination of
   `phi0 = 2/c - e^{-c(h-h*)}(2/c + h - h*)`) delivered the floor only
   within the reach `h >= h* - 2/c`, and searching exactly where the
   proof stops found a violating design: an asymmetric near-anchor
   cluster plus one below-reach probe of weight 3e-4, ratio 0.8517
   (verified at 50-digit precision). The old scan missed it because its
   weight grid never went below 0.05 and its random search never found
   the asymmetric cluster. Structure-proofness is a REACH phenomenon;
   A4's trust region (`r <= 2/c`) is exactly what keeps the agent inside
   it. See `results/OPEN1.md` section C-deep and THEOREMS.md T9.
...
| Real-data port (realdata) | 10/10 cells reproduce the published run | the published REFLEX calibration |
| Floor vs structure (T9) | within-reach min 1.0005; witness beyond 0.8517 | reach h*-2/c is exact |
...
samples the cycle at an arbitrary phase. The tail average is stable within 0.01 under horizon perturbation
(worst measured spread 0.0062; see results/STABILITY.md).

## Relation to the paper and to REFLEX

The math lives in the companion `mlxor-derivations` folder (derivations,
theorem register, proofs, 38-check numerical suite); this repo is the
