...
class Cell:
    ...
    def __init__(self, rating, regime, h_dec, A, k_raw, sigma_ann):
        ...
        self.inten = TOXIC_INTENSITY_RATIO * A
        self.inten_b = TOXIC_BASE_RATIO * A
    ...
    def tau(self, h):
        return self.gbar * (self.inten_b + ALPHA * FEEDBACK * self.inten
                            * np.exp(-self.c_t * h))

    def eps(self, h):
        return self.gbar * ALPHA * FEEDBACK * self.inten * self.c_t \
            * np.exp(-self.c_t * h)
...
def main():
    ...
    ap.add_argument("--reflex-root", default=None,
                    help="path to the calibration data released with "
                         "REFLEX (arXiv:2608.16155)")
    args = ap.parse_args()
    if args.reflex_root is None or not os.path.isdir(
            os.path.join(args.reflex_root, "endo_market_v4")):
