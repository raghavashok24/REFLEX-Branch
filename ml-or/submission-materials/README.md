# The Price of Self-Knowledge - complete paper repository
...
|---|---|---|
| `mlxor-derivations/` | The mathematical foundation: 9 derivation documents, the theorem register (25 results), full LaTeX appendix proofs, notation/assumption register, open-problems register (OPEN-1 now carrying a numerical premise check), novelty crosswalk, symbol->REFLEX map | 34/34 numerical checks + document-consistency suite, both in CI |
...
deploy-observe-fit-correct loop. **Ten measurement-forced pivots are
...
ablation fix, and the structure-proof exchange-rate floor (OPEN-1). Each
made the theory sharper; none weakened a claim.

Headline additions beyond the original five experiments:

- **Baselines (E6).** No published baseline Pareto-dominates SafeD-PerfGD
  on (final error, cumulative regret); the unsafe finite-difference
  gradient baseline pays >5x SafeD's regret. Raw regret alone is won by
  UCB-Grid - deliberately reported, because certification time is a real
  cost and that purchase is the paper's thesis.
...
- **d dimensions (E10).** Gamma_PO^{-1/2}-shaped exploration beats
  ...
  curvature-dispersed universe (risk ratio 1.21, static F = 1.45), and is
...
- **OPEN-1 premise check.** The exchange-rate floor is structure-proof:
  even a policy that knows the response family exactly cannot beat
  (1/2) gamma_PO sigma^2 (min Cramer-Rao ratio 1.005 over designs;
  tight-side probing is monotonically worse). The register's OPEN-1 entry
  is upgraded with this evidence.
