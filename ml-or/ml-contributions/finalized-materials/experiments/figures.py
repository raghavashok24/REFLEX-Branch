...
from posk.estimators import StructuralFit, secant_slope  # noqa: E402
...
def fig_baselines(fast):
    ...
    for r_ in rows:
        e, reg = float(r_["final_err"]), float(r_["cum_regret"])
        is_safe = r_["baseline"] == "SafeD-PerfGD"
        ax.scatter(max(e, 1e-4), max(reg, 1e-1),
                   s=90 if is_safe else 50,
                   color="#d62728" if is_safe else "#1f77b4",
                   zorder=3 if is_safe else 2)
        ax.annotate(r_["baseline"], (max(e, 1e-4), max(reg, 1e-1)),
                    textcoords="offset points", xytext=(6, 4), fontsize=8)
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlabel(r"final error $|h_T - h_{PO}|$")
    ax.set_ylabel("cumulative performative regret")
    ax.set_title("Safety and identification are purchased, not free (E6):\n"
                 "no baseline Pareto-dominates SafeD-PerfGD")
