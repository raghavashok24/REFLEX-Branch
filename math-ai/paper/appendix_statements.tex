...
by symmetric probing with least squares. The argument is a van Trees
reduction in the style of \citet{gill1995applications}.
\end{theorem}
...
$M$, the A-optimal design in the sense of the classical design theory of
\citet{kiefer1960equivalence} and \citet{pukelsheim2006optimal} is
$M^{*} = (B/\tr\,\Gpo^{1/2})\,\Gpo^{-1/2}$, with value
$(\tr\,\Gpo^{1/2})^2/B$: explore where the objective is flat. Pricing the
design in the experimenter's own objective inverts the conclusion that naive
exploration is optimal for the linear quadratic regulator
\citep{simchowitz2020naive,wagenmaker2021task,dean2020sample}.
\end{theorem}
...
Extended Chebyshev systems and their Fisher-information consequences are
standard in the design literature \citep{pukelsheim2006optimal}; the content
below is that noiseless retraining trajectories fall on the wrong side of the
support threshold.
...
$\delta_{\mathrm{mis}}$ wins, to leading order, which sharpens the message of
\citet{lin2024plugin} that misspecified structural models still help into a
budget-explicit rule, if and only if
...
equality points $\{\hstar,\ \hstar - 2/c\}$, which is every estimable design,
since a design supported on those two points alone leaves $g$ outside the
range of $M(\mu)$; and $\inf_\mu R(\mu) = 1$,
