...
That division of labour has precedent. Computation has guided mathematical
intuition toward theorems people then proved \citep{davies2021advancing},
search has produced explicit counterexamples to conjectures
\citep{wagner2021constructions} and new constructions out of program spaces
\citep{romera2024mathematical}, and proof systems now solve olympiad problems
end to end \citep{trinh2024solving,hubert2025olympiad}, on a lineage of
machine-assisted proof and experimental mathematics running back to the
four-colour theorem
\citep{appel1977every,borwein2008mathematics,hales2017formal}. What we add is
the failure direction, recorded completely: what a search wrongly certified,
why the proof could not reach it, and how the proof's own stopping point
located the refutation. Appendix \ref{app:related} places the episode against
that work.
...
\textbf{What this paper is not.} It is not an autonomous mathematical agent in
the sense of \citet{hubert2025olympiad}, and it does not search a space of
proofs; we make no claim in either direction. The proofs here were written by
people, and none is formalized in a proof assistant \citep{mathlib2020}. What was automated is numerical checking against declared tolerances,
...
own doing; that channel is the dealer's adverse-selection problem
\citep{gueant2013inventory}, and the fact that its own quotes feed the flow it
then reads is the price-reading effect of \citet{barzykin2026quoting}. The
structural market used throughout is that of REFLEX \citep{reflex2026}. This
is performative prediction
...
\citep{izzo2021learn,miller2021outside,mendler2020stochastic,drusvyatskiy2022stochastic,li2022state,brown2022performative}
all need an estimate of how the distribution responds to the action, and they
get it by perturbing deployments. Those perturbations are deployments of a real
...
(T3, T4), by van Trees arguments in the style of \citet{gill1995applications}.
The cheapest way to spend a multidimensional budget is to probe where the
objective is flat (T5a, C5.1), which inverts the classical design canon
\citep{kiefer1960equivalence,pukelsheim2006optimal} by pricing the design in
the experimenter's own objective, and inverts ``naive exploration is optimal''
for the linear quadratic regulator \citep{simchowitz2020naive}. And a deployed
agent can be made to correct only when a confidence sequence certifies that
correcting cannot destabilize the retraining loop (L4, P7.1). Pricing
experimentation inside the objective it serves is the dual control problem
\citep{feldbaum1960dual,bombois2006least}; what is new here is that the plant
is a retraining learner. The theory was also instantiated
...
$R(\mu) > 1$ strictly, and $\inf_\mu R(\mu) = 1$, approached by symmetric
designs collapsing at the anchor but not attained. The one configuration on
which the underlying inequality holds with equality, mass only on
$\{\hstar,\ \hstar - 2/c\}$, does not identify the estimand. Within reach,
knowing the family cannot beat the floor. (ii) The reach is exact: the four-point witness above attains
$R = 0.8517$.
\end{theorem}
...
person deciding where to look. It does add one thing to the line of work in
which search refutes a conjecture outright
\citep{wagner2021constructions,romera2024mathematical}: here the search had
already run and returned the wrong verdict, and what redirected it was a
proof's failure rather than a larger budget. The eleventh pivot needed a proof
to produce the region, and no further search inside the original space would
have found it.
