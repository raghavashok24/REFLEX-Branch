...
| L2 | Exploitation-information lemma | `E[gain] <= g1 delta^{3/2} T^{1/2} (E S_T)^{3/4} / sigma^{1/2}` via the exact identity `E|E[s|F_t]| = TV_t` + Pinsker `TV_t <= delta sqrt(E S_t)/sigma`; at the minimax scale this is an `O(T^{-1/2})` fraction of the cost - exploitation is bounded by information already purchased | A2-adaptive, A3, A4, CE anchor | D3 sec. 3 | V3.3 | PV |
| T4 | Minimax lower bound, value budget | At the minimax scale, `inf sup Var x C_T >= gamma_PO sigma^2 / 27 (1 - O(T^{-1/2}))` (Le Cam, c* = 2/3); the sharp constant `1/2` is conjectured (OPEN-1) | A1, A2-adaptive, A3, A4, A5 | D3 sec. 3 | V3.4 | PV* (two-point family; proofs.tex establishes the constant-factor version `>= gamma_PO sigma^2/27 (1 - O(T^{-1/2}))` rigorously; the sharp constant 1/2 -- numerically supported -- and general priors = OPEN-1) |
...
| T9 | Structure-proofness within reach | Within `h >= h* - 2/c`, no family-knowing design's Cramer-Rao product beats the floor: `R(mu) >= 1`, strict off the two equality points `{h*, h* - 2/c}`, `inf = 1` in the collapsing limit, via the pointwise domination of `phi0 = 2/c - e^{-c(h-h*)}(2/c + h - h*)`; the reach is exact - an explicit witness with one below-reach probe (weight 3e-4) attains `R = 0.8517` (50-digit verified) | A1 (local cost); A4 with `r <= 2/c` | D3 sec. 5 | V9.1-V9.4 | PV |

**Check accounting.** V0: 3, V1: 4, V2: 5, V3: 4, V4: 5, V5: 1, V6: 2,
V7: 6, V8: 4, V9: 4 - total **38**, matching `verify/last_run.log` (38/38 PASS).
...
d-dimensional; T9 stands on the D5 sensitivity system and closes the
structure-proofness question in both directions (proved within the reach
`2/c`, refuted beyond it by the frozen witness). No circularities; every result's assumptions are a subset of
A1-A6 + the two anchoring conventions.

**Paper mapping (4-page cut).** Body theorems as compiled: Theorem 1 = T2,
Theorem 2 = T3/T4 (with the L2 mechanism), Theorem 3 = T9; T1 + C1.1,
T5 + C5.1, and T6-T8 appear as in-text results. Appendix: everything, in
this register's order. Journal delta: OPEN-1, OPEN-2 (see
`OPEN-PROBLEMS.md`).
