...
from posk import (BlindRRM, Market, SafeDPerfGD,   # noqa: E402
...
# Reported "final" errors are tail averages over the deployed path (the last
# TAIL steps), never last-iterate readings: unanchored cells limit-cycle, and
# a last iterate samples the cycle at an arbitrary phase (pivot record: two
# e7 cells moved 2-4x under a one-step horizon perturbation; the tail average
# pins every cell to three decimals and sharpens the T7 signature).
TAIL = 300


def tail_err(path, target, tail=TAIL):
    return float(abs(np.mean(np.asarray(path)[-tail:]) - target))
...
def e4_design_identifies_curvature(fast=False):
    ...
    row("E4", "SafeD-PerfGD settles at the performative optimum",
        float(np.mean(path[-TAIL:])), hpo, 0.08)
...
def e6_baselines(fast=False):
    ...
    Metrics: final distance to h_PO (tail average over the deployed path,
    same metric for every arm) and cumulative performative regret vs the
    optimum (oracle-measured), all at a common deployment horizon.
    Every arm runs over a common bank of seeds; the table reports the
    median with an interquartile range on both axes, and the Pareto
    criterion is checked at the medians AND seed-by-seed (matched seeds).
    Pivot record: the first version scored one arm with a 100-step tail
    average and the rest with last iterates, from a single seed each - an
    inconsistent (and single-draw) comparison. One metric, many seeds."""
    ...
    def one_seed(seed):
        ...
        for _ in range(T):
            ...
            ph.append(h)
        out["ZO-PerfOpt"] = (tail_err(ph, hpo),
                             float(np.sum(mkt.Phi(hpo)
                                          - mkt.Phi(np.asarray(ph)))))
        ...
        for _ in range(T):
            ...
            ag.observe(h, y)
            ph.append(h)
        out["UCB-Grid"] = (tail_err(ph, hpo),
                           float(np.sum(mkt.Phi(hpo)
                                        - mkt.Phi(np.asarray(ph)))))
        # blind for the table
        env = StructuralEnv(mkt, seed=seed)
        path = run_agent(BlindRRM(mkt, hs), env, T)
        out["BlindRRM"] = (tail_err(path, hpo),
                           float(np.sum(mkt.Phi(hpo) - mkt.Phi(path))))
        return out
    ...
    med = {}
    lines = ["baseline,err_median,err_q1,err_q3,"
             "regret_median,regret_q1,regret_q3,n_seeds"]
    for k in arms:
        errs = np.array([o[k][0] for o in per_seed])
        regs = np.array([o[k][1] for o in per_seed])
        med[k] = (float(np.median(errs)), float(np.median(regs)))
        lines.append("%s,%.4f,%.4f,%.4f,%.2f,%.2f,%.2f,%d"
                     % (k, np.median(errs), *np.percentile(errs, [25, 75]),
                        np.median(regs), *np.percentile(regs, [25, 75]),
                        len(seeds)))
        print("    %-14s median |h-h_PO| = %.4f [%.4f, %.4f]   "
              "median regret = %8.1f [%8.1f, %8.1f]"
              % (k, np.median(errs), *np.percentile(errs, [25, 75]),
                 np.median(regs), *np.percentile(regs, [25, 75])))
    with open(os.path.join(RESULTS_DIR, "e6_baselines.csv"), "w") as f:
        f.write("\n".join(lines) + "\n")
    best_err = min(v[0] for v in med.values())
    row("E6", "SafeD median final error within 1.5x of best baseline "
        "(%d seeds)" % len(seeds),
        float(med["SafeD-PerfGD"][0] <= 1.5 * max(best_err, 1e-6) + 0.05),
        1.0, 0.0)
    ...
    e0, r0 = med["SafeD-PerfGD"]
    dominated_med = any(v[0] <= e0 and v[1] <= r0
                        for k, v in med.items() if k != "SafeD-PerfGD")
    dom_detail = []
    for seed, o in zip(seeds, per_seed):
        es, rs = o["SafeD-PerfGD"]
        for k, v in o.items():
            if k != "SafeD-PerfGD" and v[0] <= es and v[1] <= rs:
                dom_detail.append((seed, k))
    n_dom_seeds = len(set(s for s, _ in dom_detail))
    print("    (seed-by-seed: SafeD dominated in %d of %d seeds%s)"
          % (n_dom_seeds, len(seeds),
             "; " + ", ".join("seed %d by %s" % d for d in dom_detail)
             if dom_detail else ""))
    with open(os.path.join(RESULTS_DIR, "e6_baselines.csv"), "a") as f:
        f.write("# seed-by-seed Pareto: SafeD dominated in %d of %d seeds%s\n"
                % (n_dom_seeds, len(seeds),
                   "; " + "; ".join("seed %d by %s" % d for d in dom_detail)
                   if dom_detail else ""))
    # Preregistered at the medians; the seed-by-seed count is reported (in
    # the claim text, the console, and the CSV), not asserted - an honest
    # frontier that flips in a seed is a finding, not a failure.
    row("E6", "no baseline Pareto-dominates SafeD on median "
        "(error, regret) over %d seeds (dominated in %d individual seeds)"
        % (len(seeds), n_dom_seeds),
        float(not dominated_med), 1.0, 0.0)
    med_fd, med_sd = med["FD-PerfGD"][1], r0
    row("E6", "unsafe baseline (FD-PerfGD) pays >5x SafeD median regret",
        float(med_fd > 5 * med_sd), 1.0, 0.0)
    ...
    for ud in (True, False):
        for ua in (True, False):
            for ug in (True, False):
                ...
                path = run_agent(ag, env, T)
                # tail average, not last iterate: the anchor-off cells
                # limit-cycle, and a last-iterate reading moves 2-4x under
                # a one-step horizon change (pivot record; see TAIL above)
                res[(ud, ua, ug)] = dict(
                    err=tail_err(path, hpo),
...
def e8_pricing_domain(fast=False):
    ...
    row("E8", "transplanted SafeD reaches the pricing optimum",
        float(np.mean(pp[-TAIL:])), pm.p_po, 0.08)
...
def main():
    ...
    lines = ["# Pipeline results vs theory", "",
             "Tolerances are preregistered; the rel. gap column reports "
             "where the measurement actually landed inside the band.", "",
             "| Exp | Claim | Measured | Predicted | Rel. gap | Tol | Status |",
             "|---|---|---|---|---|---|---|"]
    n_fail = 0
    for exp, claim, meas, pred, tol, status in ROWS:
        n_fail += status == "FAIL"
        gap = abs(meas - pred) / max(abs(pred), 1e-12)
        lines.append("| %s | %s | %.5g | %.5g | %.1f%% | %g%% | %s |"
                     % (exp, claim, meas, pred, 100 * gap, 100 * tol, status))
