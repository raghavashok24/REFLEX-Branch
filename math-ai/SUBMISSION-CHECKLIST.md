...
4. **0 overfull boxes, 0 undefined references, 0 undefined citations, 0 LaTeX
   ...
   then `pdflatex` twice. One underfull hbox and two underfull vbox page-fill
   notices remain; these are page-break reports, and the affected pages render
   without visible defect.
5. **Every section heading has content under it.** Checked by extracting the
   ...
   consecutive appendix headings, after a first build in which Table 1 floated
   past Appendix D and left Appendix C empty. Table 1 is now pinned with
   `[H]` from the `float` package, under its own heading, preceded by a
   `\clearpage`.
...
8. **Author names in the PDF text: 2 occurrences, both correct.** One is the
...
11. **`prose_lint.py` from `anthropic-skills:prose-guard` is clean** on
    `main.tex`, `appendix_statements.tex` and `appendix_proofs.tex` at
    `--fail-on medium`.
12. **Every number in the body traces to a committed artifact.** Traced by
...
13. **The pipeline and derivation suites were not re-run.** They were executed
